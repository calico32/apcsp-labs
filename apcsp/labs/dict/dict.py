...
def add_definition(
    dictionary: Dict[str, List[Definition]], word: str, definitions: List[List[str]]
) -> None:
    ...
    while True:
        ...
        if user_input.split(" ")[0] in ["list", "l"]:
            ...
            for i in range(page * 10 - 10, page * 10):
                ...
                print(
                    f"{Fore.BLUE}{i + 1:>3}:"
                    f" {list(dictionary.keys())[i]}{Style.RESET_ALL}"
                )
        ...
        if user_input.split(" ")[0] in ["search", "s"]:
            ...
            for user_input in dictionary:
                if (
                    query.lower() in user_input.lower()
                    or len(
                        list(
                            filter(
                                lambda x: query.lower() in x.definition.lower(),
        ...
        if user_input.lower() == "about":
            print(
                Fore.YELLOW
                + "Toki Pona is a human language invented by Sonja Lang in 2001. Learn"
                " more at https://www.tokipona.org/."
                + Style.RESET_ALL
            )
